...
class udCesium {
  ...
  /**
   * @abstract  加载模型
   * @param {Object} udsModel uds加载对象
   * @param {Boolean} isFixedCarmer   取消相机默认定位到uds处
   * @param {Object} cameraPosition 相机定位位置
   * @returns 
   */
  loadUds(udsModel, isFixedCarmer, cameraPosition) {
    if (Cesium) {
  ...
  /**
   * @abstract  加载模型
   * @param {Array} list uds加载对象列表数组
   * @param {Boolean} isFixedCarmer 取消相机默认定位到uds处
   * @param {Object} cameraPosition 相机定位位置
   * @returns 
   */
  async loadUdsList(list, isFixedCarmer, cameraPosition) {
    if (Cesium) {
      let results = []
      if (!this.udScenePrimitive) {
        //viewer.scene.globe.depthTestAgainstTerrain = true;
  ...
  clearUds() {
    udSDKJS_RenderQueueClear() //先清除掉
    this.udScenePrimitive = null // 清除uds的Primitive
